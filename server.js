...
app.get('/statistic', (req, res) => {
    res.sendFile(join(__dirname, 'statistic.html'));
});
...
async function getPlayers(roomkey) {
    let players = db.all("SELECT playerId, username, gender, creator FROM lobbys where roomkey = ?", [roomkey]);
    return players;
}

async function getRooms() {
    let rooms = db.all("SELECT roomKey, roomName, state FROM rooms WHERE state = 0 AND started = 0");
    return rooms;
}
...
io.on('connection', async (socket) => {
    ...
    //Statistic Section
    socket.on('getStatistic', async (id, callback) => {
        console.log("Stat");
        await db.each('SELECT login, numbGames, numbBusfahrer, givenSchluck, recievedSchluck, selfSchluck, numbEx FROM players WHERE id = ?', [id], (err,row) => {
            db.each('SELECT * FROM globalStat', [], (err,res) => {
                io.to(socket.id).emit('setStatistic', res, row);
            });
        });
        callback();
        return;
    ...
    socket.on('createRoom', async (roomKey, user, state, callback) => {
        ...
        try {
            result = await db.run('INSERT INTO rooms (roomkey, roomName, state, started) VALUES (?, ?, ?, 0)', roomKey, roomKey, state);
        } catch(e) {
            if(e.errno === 19) {
                callback();
            }
    ...
    socket.on('updateLobby', async(roomKey, callback) => {
        ...
        io.to(roomKey).emit('changeLobby', players, roomKey);
        callback();
    });

    socket.on('removePlayer', async(room, id, callback) => {
        await db.run('DELETE FROM lobbys WHERE roomkey = ? AND playerId = ?;', [room, id]);

        io.to(room).emit('setDeletePlayer', id);
        io.to(room).emit('checkRemove', id);
        callback();
    });

    socket.on('getRemove', async(user, id, callback) => {
        var token = getRoomToken(user);
        var numb = Number(id);
        if(token.id === numb) {
            io.to(socket.id).emit('setRemove');
        }
        callback();
    })
    ...
    socket.on('quitGame', async(user) => {
        var token = getRoomToken(user);

        await db.run('DELETE FROM lobbys WHERE roomkey = ? AND playerId = ?', [token.room, token.id]);
        await db.run('DELETE FROM cards WHERE roomkey = ? AND playerId = ?', [token.room, token.id]);

        io.to(socket.id).emit('setQuitGame');
    });

    socket.on('stopGame', async(user) => {
        var token = getRoomToken(user);

        await db.run('DELETE FROM lobbys WHERE roomkey = ?', [token.room]);
        await db.run('DELETE FROM cards WHERE roomkey = ?', [token.room]);
        await db.run('DELETE FROM phase1 WHERE roomkey = ?', [token.room]);
        await db.run('DELETE FROM phase2 WHERE roomkey = ?', [token.room]);
        await db.run('DELETE FROM phase3 WHERE roomkey = ?', [token.room]);
        await db.run('DELETE FROM rooms WHERE roomKey = ?', [token.room]);

        io.to(token.room).emit('setStopGame');
    });
    ...
    socket.on('createCards', async(user, callback) => {
        ...
        await db.run('UPDATE rooms SET started = 1 WHERE roomKey = ?', [token.room]);
        updateGames();
